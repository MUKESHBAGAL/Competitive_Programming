...
int main() {
    int n;
    int i, j;
    printf("Enter number of row Number Characters '5': ");
    scanf("%d", &n);
    ...
    for (i = 1; i <= n; i++) {
        ...
        for (j = 1; j <= n; j++) {
            ...
            if (i == 1 && j < n - 1 ||             // Top horizontal line
                j == 1 && i <= num ||              // Left vertical line
                i == num && j < n - 1 ||           // Middle horizontal line
                j == n - 1 && (i > num && i < n) || // Right vertical line
...
// // // OUTPUT::
// Enter number of row Number Characters '5': 7
