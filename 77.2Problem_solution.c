...
int main() {
    int n;
    int i, j;
    printf("Enter number of row Number Characters '9': ");
    scanf("%d", &n);
    ...
    for (i = 1; i <= n; i++) {
        ...
        for (j = 1; j <= n; j++) {
            ...
            if (i == 1 && (j>1&&j < n - 2) ||             // Top horizontal line
                j == n-2 && i<n||              // Left vertical line
                i == num+1 && (j>1&&j < n - 1) ||           // Middle horizontal line
                j == 1 && (i < num+1 && i >1) || // Right vertical line
                i == n && j < n -2          // Bottom horizontal line
               )
                printf("* ");
            else
...
// OUTPUT::
// Enter number of row Number Characters '9': 7
